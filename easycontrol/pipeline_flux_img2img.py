...
class FluxImg2ImgPipeline(DiffusionPipeline, FluxLoraLoaderMixin, FromSingleFileMixin, FluxIPAdapterMixin):
    ...
    def prepare_latents(
        ...
        device,
        generator,
        subject_image=None,  # 新增：主题图像
        condition_image=None,  # 新增：条件图像
        cond_number=1,
        sub_number=1,
        latents=None,
    ):
        ...
        # position_encoding I2I maybe not this empty encoding
        ...
        # noise_latent_image_ids=latent_image_ids
        ...
        # return latents, latent_image_ids        # original return
        return cond_latents, latent_image_ids, latents
    ...
    def __call__(
        ...
        max_sequence_length: int = 512,
        spatial_images: List[PipelineImageInput] = [],
        subject_images: List[PipelineImageInput] = [],
        cond_size: int = 512,
    ):
        r"""
        ...
        if sub_number > 0:
            ...
            for img in subject_images:
                img = self.image_processor.preprocess(img, height=cond_size, width=cond_size)
                img = img.to(dtype=torch.float32)
                subject_image_ls.append(img)
            subject_image = torch.cat(subject_image_ls, dim=-2)  # 沿高度拼接
        else:
            subject_image = None
        ...
        if cond_number > 0:
            ...
            for img in spatial_images:
                img = self.image_processor.preprocess(img, height=cond_size, width=cond_size)
                img = img.to(dtype=torch.float32)
                cond_image_ls.append(img)
            condition_image = torch.cat(cond_image_ls, dim=-2)
        else:
