...
class EasyControlLoadFlux:
    ...
    def load_model(self, diffuser_model,load_8bit, cpu_offload, hf_token=None):
        # login(token=hf_token)
        prefixes = folder_paths.folder_names_and_paths["diffusion_models"][0]
        for prefix in prefixes:
            if os.path.exists(os.path.join(prefix, diffuser_model)):
                model_path = os.path.join(prefix, diffuser_model)
                break
        
        cache_dir = folder_paths.get_folder_paths("diffusers")[0]
        print(cache_dir)
        if load_8bit:
            quant_config_t5 = BitsAndBytesConfig(load_in_4bit=True,bnb_4bit_compute_dtype=torch.float16)
            quant_config = BitsAndBytesConfig(load_in_8it=True, bnb_4bit_compute_dtype=torch.float16)
        else:
        ...
        pipe = FluxPipeline.from_pretrained(model_path, transformer=transformer, text_encoder_2=text_encoder, torch_dtype=torch.bfloat16, cache_dir=cache_dir)
        # pipe.to("cuda")
...
class EasyControlLoadLora:
    ...
    def load_lora(self, pipe, lora_name, lora_weight, cond_size):
...
class EasyControlLoadMultiLora:
    ...
    def load_multi_lora(self, pipe, lora_name1, lora_weight1, lora_name2, lora_weight2, cond_size):
        device = "cuda" if torch.cuda.is_available() else "cpu"
        transformer=pipe.transformer
...
class EasyControlGenerate:
    ...
    def generate(self, pipe, prompt, prompt_2, height, width, guidance_scale, 
